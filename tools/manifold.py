"""
Инструменты для построения manifold на данных о поведении скважин.

Файл отвечает только за построение низкоразмерной карты поведения с помощью
UMAP. Поддерживаются два режима: базовый по евклидовой метрике и уточнённый,
где расстояния между ближайшими рядами пересчитываются с помощью FastDTW.

Ключевые функции:
  - ``embed_umap_euclid`` — быстрый UMAP по выровненным временным рядам.
  - ``embed_umap_fastdtw`` — UMAP с доуточнением расстояний FastDTW.
  - ``_fastdtw_multichannel`` — DTW для многоканальных временных рядов.
  - ``_flatten_series_matrix`` — подготовка тензора ``[N, T, C]`` для UMAP.
